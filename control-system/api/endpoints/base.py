...
AllowedRoutes = Literal['get_all_paginated', 'get_by_id', 'create', 'update', 'bulk_update', 'delete']

class BulkUpdateSchema(BaseModel):
    ids: list[int]

class CRUDRouter[InstanceSchema: BaseModel, CreateSchema: Optional[BaseModel], UpdateSchema: Optional[BaseModel]]():
    def __init__(self, 
    ...
    def setup_get_all_paginated(self):
        ...
        async def get_all(
            ...
            return service.get_all_paginated(request, pagination.page, pagination.page_size)

    def setup_get_by_id(self): 
        @self.router.get("/{item_id}", response_model=InstanceSchema)
        async def get_by_id(item_id: int, service: BaseCRUDService[InstanceSchema] = Depends(self.service_factory)):
            return service.get_by_id(item_id) 

    def setup_create(self):
        @self.router.post("/", response_model=InstanceSchema)
        async def create(item: CreateSchema, service: BaseCRUDService[InstanceSchema] = Depends(self.service_factory)):
            return service.create(item)

    def setup_update(self):
        @self.router.put("/{item_id}", response_model=InstanceSchema)
        async def update(item_id: int, item: InstanceSchema, service: BaseCRUDService[InstanceSchema] = Depends(self.service_factory)):
            return service.update(item_id, item)

    def setup_bulk_update(self):
        @self.router.post("/bulk_update", response_model=int)
        async def update(body: BulkUpdateSchema = Body(), service: BaseCRUDService[InstanceSchema] = Depends(self.service_factory)):
